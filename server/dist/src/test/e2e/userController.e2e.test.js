...
const dbTestUtils_1 = require("../utils/dbTestUtils");
...
describe('UserController E2E Tests', () => {
    ...
    beforeAll(async () => {
        await (0, dbTestUtils_1.setupTestDatabase)();
        await (0, dbTestUtils_1.ensureTestDatabase)();
    });
    // Cleanup after all tests
    afterAll(async () => {
        await (0, dbTestUtils_1.teardownTestDatabase)();
    });
    // Clean dynamic data after each test (not before) to avoid race conditions
    afterEach(async () => {
        await (0, dbTestUtils_1.cleanDatabase)();
    });
    describe('POST /api/users - Register Citizen', () => {
        ...
        it('should register a new citizen successfully', async () => {
            ...
            const userExists = await userRepository_1.userRepository.existsUserByUsername(validRegistrationData.username);
        ...
        it('should create user with hashed password in database', async () => {
            ...
            const user = await userRepository_1.userRepository.findUserByUsername(validRegistrationData.username);
        ...
        it('should handle multiple concurrent registrations', async () => {
            ...
            const promises = users.map(user => (0, supertest_1.default)(app_1.default)
                .post('/api/users')
                .send(user));
            ...
            for (const user of users) {
                const exists = await userRepository_1.userRepository.existsUserByUsername(user.username);
    ...
    describe('Complete Registration and Authentication Flow', () => {
        ...
        it('should complete full registration → login → access protected route → logout flow', async () => {
            ...
            const currentUserResponse = await (0, supertest_1.default)(app_1.default)
        ...
        it('should maintain separate sessions for different users', async () => {
            ...
            await (0, supertest_1.default)(app_1.default).post('/api/users').send(user1).expect(201);
            await (0, supertest_1.default)(app_1.default).post('/api/users').send(user2).expect(201);
            ...
            const current1 = await (0, supertest_1.default)(app_1.default)
                .get('/api/sessions/current')
                .set('Cookie', cookies1)
                .expect(200);
            const current2 = await (0, supertest_1.default)(app_1.default)
                .get('/api/sessions/current')
                .set('Cookie', cookies2)
                .expect(200);
        ...
        it('should verify password is correctly hashed and verifiable', async () => {
            ...
            const user = await userRepository_1.userRepository.findUserByUsername(newUser.username);
            expect(user).not.toBeNull();
    ...
    describe('Integration with Pre-loaded Test Users', () => {
        it('should not conflict with pre-loaded test users', async () => {
            ...
            const testCitizenExists = await userRepository_1.userRepository.existsUserByUsername('testcitizen');
            const testMunicipalityExists = await userRepository_1.userRepository.existsUserByUsername('testmunicipality');
            const testAdminExists = await userRepository_1.userRepository.existsUserByUsername('testadmin');
            ...
            expect(await userRepository_1.userRepository.existsUserByUsername('testcitizen')).toBe(true);
            expect(await userRepository_1.userRepository.existsUserByUsername('testmunicipality')).toBe(true);
            expect(await userRepository_1.userRepository.existsUserByUsername('testadmin')).toBe(true);
        });
